...
const grapeExpression = function (arr) {
    for(let i=0; i<arr.length; i++) {
        if(arr[i].color === "purple") {
            return console.log(`The fruit with index ${arr.indexOf(arr[i])} is a grape`)
        } 
...
// CODE HERE
const onelineFunc = (par1, par2) => `The first parameter is ${par1}. The second parameter is ${par2}.`

// INVOKE THE FUNCTION HERE. THE PARAMETERS TAKE ANY DATATYPE.
console.log(onelineFunc(1, 2));
...
greeting('alec','wixom', ((a, b) => `Hello, my name is ${a} ${b}`))
...
function pricesPlusTax (pricesArr, callback1) {
    for (i = 0; i < pricesArr.length; i++) {
        totalCost.push (pricesArr[i] + (pricesArr[i] * .2))
    }
    return callback1(totalCost)
}
...
console.log(pricesPlusTax (prices, (a) => `The new array plus tax = ${a}`))
...
function multiplyingFactory (num1){
    return (num2) => {
        if (num1 >= 5) {
            ...
            return 'Cannot multiply: the first number is not smaller than 5'
        }
    }
}
// console.log(multiplyingFactory(4)(5));
/* 
